...
router.post('/addRating', async (req, res) => {
  ...
  if (!isValidRating(rating)) {
    return res.status(400).json({ error: 'Invalid rating, it should be a number between 1 and 5' });
  }
...
router.get('/getUserRatings/:userType/:userId/:reviewId', async (req, res) => {
  ...
  try {
    const userRatings = await ratingData.getReviewById(userId, reviewId, userType);
